...
for i in x:
    ...
    with open('output/heap' + str(i) + '.out') as file:
        for line in file:
            if ('Liczba porownan miedzy kluczami' in line):
                heapC[(i)//100 -1].append(int((line.split('kluczami: ')[1]).split()[0]))
            if ('Liczba przypisan wartosci' in line):
                heapS[(i)//100 -1].append(int((line.split('wartosci: ')[1]).split()[0]))               
...
c.plot(x,[stat.mean(heapC[i]) for i in range(10)],'r',label='Liczba porównań heap sort')
c.legend(loc="upper left")
c.set(xlabel='Wielkość tablicy', ylabel='Liczba porównań')
c.set_title('Liczba porównań dla rosnącej tablicy w zależności od wielkości (k = ' + str(k) +')')
c.grid()
...
cn.plot(x,[stat.mean(heapC[i])/((i+1)*100) for i in range(10)],'r',label='Liczba porównań / n heap sort')
cn.legend(loc="upper left")
cn.set(xlabel='Wielkość tablicy = n', ylabel='Liczba porównań / n')
cn.set_title('Liczba porównań / n dla rosnącej tablicy w zależności od wielkości (k = ' + str(k) +')')
cn.grid()
...
s.plot(x,[stat.mean(heapS[i]) for i in range(10)],'r',label='Liczba przypisań heap sort')
s.legend(loc="upper left")
s.set(xlabel='Wielkość tablicy', ylabel='Liczba przypisań')
s.set_title('Liczba przypisań dla rosnącej tablicy w zależności od wielkości (k = ' + str(k) +')')
s.grid()
...
sn.plot(x,[stat.mean(heapS[i])/((i+1)*100) for i in range(10)],'r',label='Liczba przypisań / n heap sort')
sn.legend(loc="upper left")
sn.set(xlabel='Wielkość tablicy = n', ylabel='Liczba przypisań / n')
sn.set_title('Liczba przypisań / n dla malejącej tablicy w zależności od wielkości (k = ' + str(k) +')')
sn.grid()
