...
class RBT {
    ...
    void left_rotate(Node *x) {
        ...
        if (y != nil) {
            ...
            if (y->get_left() != nil) y->get_left()->set_parent(x);
    ...
    void right_rotate(Node *x) {
        ...
        if (y != nil) {
            ...
            if (y->get_right() != nil) y->get_right()->set_parent(x);
            y->set_parent(x->get_parent());
        }
        if (x->get_parent() == nil) {
            root = y;
    ...
    void insert(int key) {
        Node* x = tree_insert(key);
        Node* y = nil;
        x->color = 'R';
        while(x != root && x->get_parent()->color == 'R') {
            if (x->get_parent() == x->get_parent()->get_parent()->get_left()) {
                y = x->get_parent()->get_parent()->get_right();
                if (y != nil && y->color == 'R') {
                    x->get_parent()->color = 'B';
                    y->color = 'B';
                    x->get_parent()->get_parent()->color = 'R';
                    ...
                    x->get_parent()->color = 'B';
                    x->get_parent()->get_parent()->color = 'R';
                    right_rotate(x->get_parent()->get_parent());
                }
            }
            else {
                y = x->get_parent()->get_parent()->get_left();
                if (y != nil && y->color == 'R') {
                    x->get_parent()->color = 'B';
                    y->color = 'B';
                    x->get_parent()->get_parent()->color = 'R';
                    ...
                    x->get_parent()->color = 'B';
                    x->get_parent()->get_parent()->color = 'R';
                    left_rotate(x->get_parent()->get_parent());
                }
            }
    ...
    void delete_fixup(Node* x) {
        while (x && x != root &&  x->color == 'B') {
            if (x == x->get_parent()->get_left()) {
                Node* w = x->get_parent()->get_right();
                if (w->color == 'R') {
                    w->color = 'B';
                    w->get_parent()->color = 'R';
                    left_rotate(x->get_parent());
                    w = x->get_parent()->get_right();
                }
                if (w->get_left()->color == 'B' && w->get_right()->color == 'B') {
                    w->color = 'R';
                    x = x->get_parent();
                }
                else {
                    ...
                    w->color = w->get_parent()->color;
                    w->get_parent()->color = 'B';
                    w->get_right()->color = 'B';
                    left_rotate(x->get_parent());
                    x = root;
                }
                ...
                if (w->get_right()->color == 'B' && w->get_left()->color == 'B') {
                    w->color = 'R';
                    x = x->get_parent();
                }
                else {
