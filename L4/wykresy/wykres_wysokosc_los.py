...
for i in x:
    with open('output/bst_los' + str(i) + '.out') as file:
        for line in file:
            if ('Srednia wysokosc:' in line):
                #print(line)
                bstS[(i)//10000 -1].append(float((line.split('wysokosc: ')[1]).split()[0]))
            if ('Max wysokosc:' in line):
                #print(line)
                bstM[(i)//10000 -1].append(float((line.split('wysokosc: ')[1]).split()[0]))              
for i in x:
    with open('output/rb_los' + str(i) + '.out') as file:
        for line in file:
            if ('Srednia wysokosc:' in line):
                #print(line)
                rbS[(i)//10000 -1].append(float((line.split('wysokosc: ')[1]).split()[0]))
            if ('Max wysokosc:' in line):
                #print(line)
                rbM[(i)//10000 -1].append(float((line.split('wysokosc: ')[1]).split()[0]))              
for i in x:
    with open('output/splay_los' + str(i) + '.out') as file:
        for line in file:
            if ('Srednia wysokosc:' in line):
                #print(line)
                splayS[(i)//10000 -1].append(float((line.split('wysokosc: ')[1]).split()[0]))
            if ('Max wysokosc:' in line):
                #print(line)
                splayM[(i)//10000 -1].append(float((line.split('wysokosc: ')[1]).split()[0]))             
...
m.set(xlabel='Wielkość danych wejściowych', ylabel='Maksymalna wysokość drzewa')
m.set_title('Maksymalna wysokość drzewa po operacji\n w zależności od wielkośći danych (próba statystyczna 20)\n input losowy')
m.grid()
...
s.set(xlabel='Wielkość danych wejściowych', ylabel='Średnia wysokość drzewa')
s.set_title('Średnia wysokość drzew po operacjach\n w zależności od wielkośći danych (próba statystyczna 20)\n input losowy')
s.grid()
