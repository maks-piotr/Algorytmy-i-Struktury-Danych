...
cn.plot(x,[stat.mean(quick_wC[i])/(((i+1)*100)*((i+1)*100)) for i in range(100)],'g',label='Liczba porównań / n^2 quick')
cn.legend(loc="upper left")
cn.set(xlabel='Wielkość tablicy', ylabel='Liczba porównań')
cn.set_title('Liczba porównań dla złośliwej tablicy / n^2 w zależności od wielkości (m = ' + str(m) + ', k losowe)')
cn.grid()
...
sn.plot(x,[stat.mean(quick_wS[i])/(((i+1)*100)*((i+1)*100)) for i in range(100)],'g',label='Czas wykonywania / n^2 quick')
sn.legend(loc="upper left")
sn.set(xlabel='Wielkość tablicy', ylabel='Czas wykonywania')
sn.set_title('Czas wykonywania dla złośliwej tablicy / n^2 w zależności od wielkości (m = ' + str(m) + ', k losowe)')
sn.grid()
