...
cn.plot(x,[stat.mean(quick_select_wC[i])/(((i+1)*100)*math.log2((i+1)*100)) for i in range(100)],'r',label='Liczba porównań / n log n quick select')
...
cn.set_title('Liczba porównań dla złośliwej tablicy / n log n w zależności od wielkości (m = ' + str(m) + ', k losowe)')
cn.grid()
...
sn.plot(x,[stat.mean(quick_select_wS[i])/(((i+1)*100)*math.log2((i+1)*100)) for i in range(100)],'r',label='Czas wykonywania / n log n quick select')
...
sn.set_title('Czas wykonywania dla złośliwej tablicy / n log n w zależności od wielkości (m = ' + str(m) + ', k losowe)')
sn.grid()
